...
export function Contact() {
  return (
    <section id="contact" className="mx-auto max-w-6xl px-6 py-20 md:py-28">
      ...
      <h2 className="reveal mb-6 text-3xl font-semibold tracking-tight md:text-4xl">
        Let&rsquo;s talk
      </h2>
      ...
      <div className="reveal flex flex-wrap gap-3" style={{ transitionDelay: "100ms" }}>
        <ContactLink
          ...
          label={SITE.email}
        />
        <ContactLink href={SITE.linkedin} icon={<LinkedInIcon />} label="LinkedIn" external />
        <ContactLink href={SITE.github} icon={<GitHubIcon />} label="GitHub" external />
      </div>
    </section>
  );
}
...
function ContactLink({
  ...
  return (
    <a
      ...
      className="inline-flex items-center gap-2.5 rounded-full border border-[var(--color-border)] bg-[var(--color-bg-elev)] px-5 py-3 text-sm text-[var(--color-fg)] transition-colors hover:border-[var(--color-accent)] hover:text-[var(--color-accent)]"
    >
      {icon}
      {label}
    </a>
