...
GabyGPT. Gaby's most recent build. A private assistant running a stock Qwen3.5-4B with no hosted API anywhere in the request path. As of v0.3 it runs on two devices from one shared core: the desktop over LM Studio, and Android with the model executing on the phone's own silicon via llama.rn — the phone half needs no desktop, no server and no network, so it works in airplane mode with the PC switched off. The architecture is the point, not the model: system policy, persistent memory and the conversation are kept deliberately separate and separately versioned. Because Qwen3.5 rejects a second system message, policy and memory ride in one, with policy first and memory explicitly framed as context that does not override it — without that framing a remembered preference can quietly outrank a behavioural rule. It is built to be measured: a frozen 41-case benchmark is the baseline, a separate memory benchmark tests recall, and a contamination gate keeps eval cases out of the training corpus. Corrections are captured in-session and reviewed by hand before they land, and forgetting a memory archives it rather than destroying it, so the memory state stays auditable. Gaby is explicit that it is not a fine-tuned model: the weights are stock and the behaviour comes from policy, memory and context — keeping that distinction sharp is what would let him later measure prompting against an actually fine-tuned version. TypeScript and Node. Private repository, so there is no public code link.

Universal PDF-to-Excel Converter. Converts any PDF — born-digital, scanned, or hybrid — into structured, Excel-ready tables. Handles the cases that break naive extractors: unruled tables with no grid lines, tables continuing across page breaks, merged cells, and scanned pages needing OCR before there is any text to parse. Output is a real .xlsx workbook rather than a CSV dump: one sheet per table, frozen bold headers, auto-filter, auto-fitted columns, and currency stored as actual numbers so Excel can total a column while still displaying it formatted. The results grid behaves like a spreadsheet with drag-select, shift-click range extension, and clipboard copy that pastes cleanly into Excel. Every cell carries a confidence score and low-confidence cells are flagged for review, because the damaging OCR failure mode is not missing data but wrong data that looks correct. It began as a way to take the tedium out of a repetitive data-entry task at work, and a few colleagues on his team use it now as well — it is a tool with real users, not just a personal build. If asked for specifics about internal systems or processes at his employer, do not speculate or name them; say the tool speeds up a manual data-entry workflow and leave it there. Python, FastAPI, PyMuPDF, pdfplumber, Tesseract OCR, OpenCV. Live at pdf-to-excel-converter-bq06.onrender.com — it is on a free hosting tier, so the first request may take a few seconds to wake the server. The repository is private.
